...
namespace EMCCadastro.Relatorios
   {
   public partial class relHistorico : EMCLibrary.tplRelatorio
      {
      private const string nomeFormulario = "relHistorico";
      ...
      public relHistorico(String idUsuario, String seqLogin, String idEmpresa, String empmaster, ConectaBancoMySql pConexao)
         {
         usuario = idUsuario;
         login = seqLogin;
      ...
      private void relHistorico_Load(object sender, EventArgs e)
         {
      ...
      #region "Botões Overrides"
       ...
       public override void btnRelatorio_Click(object sender, EventArgs e)
         ...
         try
            ...
            this.dstHistorico.Tables["MyTable"].Clear();
            //obtendo resultado da consulta SQL e carregando no DataTable
            this.dstHistorico.Tables["MyTable"].Load(oBLL.Listar(drpRelatorio).CreateDataReader());
            //verifica se encontrou registros a emitir
            if (Convert.ToInt32(this.dstHistorico.Tables["MyTable"].Rows.Count) == 0)
               {
      ...
      private string DataReport()
         ...
         strSQL.Append(" IdHistorico, Descricao");
         strSQL.Append(", If(ExigeComplemento ='S', 'Sim', 'Não') as ExigeComplemento");
         strSQL.Append(" from");
         strSQL.Append(" Historico");
         strSQL.Append(" order by");
         //verifica se ordena pelo código ou descrição do Índice
         if (cboOrdenar.SelectedValue.ToString() == "1")
            strSQL.Append(" IdHistorico");
         else
