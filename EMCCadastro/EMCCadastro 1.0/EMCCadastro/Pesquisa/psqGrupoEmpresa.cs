...
{
    public partial class psqGrupoEmpresa : Form
    {
        ...
        public psqGrupoEmpresa()
        {
            InitializeComponent();
        }

        public psqGrupoEmpresa(String idUsuario, String seqLogin, String idEmpresa, String empmaster, ConectaBancoMySql pConexao, Ocorrencia pOcorrencia)
        {
            usuario = idUsuario;
            login = seqLogin;
        ...
        private void psqGrupoEmpresa_Load(object sender, EventArgs e)
        {
            Holding oHolding = new Holding();
            HoldingRN oHoldRN = new HoldingRN(Conexao, oOcorrencia, codempresa);
        ...
        private void btnPesquisa_Click(object sender, EventArgs e)
        {
            try
            {
                GrupoEmpresaRN oGrupoEmpresaRN = new GrupoEmpresaRN(Conexao, oOcorrencia, codempresa);
                DataTable dtCon;

                if (cboHolding.SelectedValue == null)
                {

                    dtCon = oGrupoEmpresaRN.pesquisaGrupoEmpresa(empMaster, EmcResources.cInt(txtIdGrupoEmpresa.Text), 0, txtNomeGrupoEmpresa.Text);
                    grdpsqGrupoEmpresa.DataSource = dtCon;
                }
                else
                {
                    dtCon = oGrupoEmpresaRN.pesquisaGrupoEmpresa(empMaster, EmcResources.cInt(txtIdGrupoEmpresa.Text), EmcResources.cInt(cboHolding.SelectedValue.ToString()), txtNomeGrupoEmpresa.Text);
                    grdpsqGrupoEmpresa.DataSource = dtCon;
                }
        ...
        private void grdpsqGrupoEmpresa_DoubleClick(object sender, EventArgs e)
        {
            retPesquisa.codempresa = empMaster;
            retPesquisa.chaveinterna = EmcResources.cInt(grdpsqGrupoEmpresa.Rows[grdpsqGrupoEmpresa.CurrentRow.Index].Cells["idgrupoempresa"].Value.ToString());
            this.Close();
        }

        private void psqGrupoEmpresa_KeyDown(object sender, KeyEventArgs e)
        {
