...
{
    public partial class relSubLocacao : EMCLibrary.tplRelatorio
    {
        private const string nomeFormulario = "relSubLocacao";
        ...
        public relSubLocacao(String idUsuario, String seqLogin, String idEmpresa, String empmaster, ConectaBancoMySql pConexao)
        {
            usuario = idUsuario;
            login = seqLogin;
        ...
        private void relSubLocacao_Load(object sender, EventArgs e)
        {
        ...
        private void listaSubLocacao()
        {
            try
            {
                ...
                SubLocacaoRN oBLL = new SubLocacaoRN(Conexao, objOcorrencia, codempresa);

                //Montando Consulta SQL para o relatório                    

                if (EmcResources.cInt(txtIdSubLocacao.Text) >= 0)
                {
                    this.dstSublocacao.Tables["Imovel"].Clear();
                    this.dstSublocacao.Tables["Evento"].Clear();
                    this.dstSublocacao.Tables["SubLocacao"].Clear();
                    this.dstSublocacao.Tables["SubLocacao"].Load(oBLL.ListaSubLocacao(txtCodigoImovel.Text, EmcResources.cInt(txtIdEvento.Text), chkImovel.Checked,
                                                                                    chkEvento.Checked, txtDataInicio.DateValue, txtDataFinal.DateValue).CreateDataReader());

                    this.dstSublocacao.Tables["Evento"].Load(oBLL.ListaSubLocImovel(txtCodigoImovel.Text, EmcResources.cInt(txtIdEvento.Text), chkImovel.Checked,
                                                                                    chkEvento.Checked).CreateDataReader());

                    this.dstSublocacao.Tables["Imovel"].Load(oBLL.ListaSubLocImovel(txtCodigoImovel.Text, EmcResources.cInt(txtIdEvento.Text), chkImovel.Checked,
                                                                                    chkEvento.Checked).CreateDataReader());


                    //verifica se encontrou registros a emitir
                    if (Convert.ToInt32(this.dstSublocacao.Tables["SubLocacao"].Rows.Count) == 0)
                    {
        ...
        private void listaSubLocEvento()
        {
            try
            {
                Empresa oEmpresa = new Empresa();
        ...
        public override void LimpaCampos()
        {
            ...
            arr.Add(new popCombo("POR EVENTO", "1"));
            arr.Add(new popCombo("POR IMÓVEL", "2"));
